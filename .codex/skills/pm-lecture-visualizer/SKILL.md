...
## Step 1：内容深度分析（全面还原，不得简化）

读取或接收讲稿后，**立即执行深度结构分析，无需等待用户确认，直接进入生成流程**。

分析要求：
- **必须完整还原讲稿的全部内容**，包括所有具名概念、方法论、例子、数据、流程步骤、对比关系、因果逻辑，不得遗漏任何细节
- **严禁只做摘要或概括**，每一个小节、每一个术语、每一个讲师强调的点都必须被提取并体现在图中
- 识别内容结构：主题 → 一级分区 → 二级概念 → 三级细节 / 举例 / 注释
- 识别关系类型：dependency（依赖）/ flow（流程）/ contrast（对比）/ hierarchy（层级）/ cycle（循环）
- 判断最佳图表类型（见 Step 2）
- 标记重点候选（5–7 个核心概念，将用金色高亮）
- 标记警示候选（易错点 / 考点，将用红色标注）
- 自动确定文件命名

分析完成后，**直接生成图表，不询问用户是否继续**。
...
**读取原稿文件名**：AI 自动从输入文件的文件名中提取编号和主题。
...
| 检查项 | 标准 |
|--------|------|
| 内容完整性 | 讲稿所有具名概念、例子、数据、步骤均已出现，一字不漏 |
| 细节还原度 | 三级节点和注释充分体现讲稿细节，不做简化或归并 |
